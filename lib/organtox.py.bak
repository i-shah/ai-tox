...
def getToxDataSet(tox,MDB=None,col_tox='tox_fp',
                  tox_pos_ds='tox_fpp1.ds',
                  tox_neg_ds='tox_fpn1.ds',
                  ):
    if not MDB: return
    Pos = np.array([i['dsstox_cid'] for i in MDB[col_tox].find({tox_pos_ds:tox,'dsstox_cid':{'$exists':1}},{'_id':0,'dsstox_cid':1})])
    Neg = np.array([i['dsstox_cid'] for i in MDB[col_tox].find({tox_neg_ds:tox,'dsstox_cid':{'$exists':1}},{'_id':0,'dsstox_cid':1})])
    n_n = len(Neg)
    n_p = len(Pos)
...
def getImbDataSubSet(Y,X,n_pos=None,n_neg=None,missing='drop',max_xy=False):
    """
    n_pos  : the number of positives 
    n_neg  : the number of negatives
    """
    ...
    n_neg = n_neg if n_n>n_neg else n_n
    n_pos = n_pos if n_p>n_pos else n_p

    # Subset 1
    I_p = np.random.randint(0,n_p,n_pos)
    I_n = np.random.randint(0,n_n,n_neg)
    CID1  = np.concatenate((Pos[I_p],Neg[I_n]))
...
def runStudyToxML(y_out,dt,n_pos,n_neg,r_seed,
    ...
    """
    np.random.seed(r_seed)
    DS = getToxDataSet(y_out,MDB,col_tox=col_tox,tox_pos_ds=tox_pos_ds,tox_neg_ds=tox_neg_ds)
    for j in range(ss_iters):
        Dss_cv= getImbDataSubSet(DS['tox'][[y_out]],DS[dt],n_pos,n_neg)
        CLF_i = getClassifiers()
        for i_ds in range(n_ds_min,n_ds_max,n_ds_step):
            runML1(Dss_cv,y_out,CLF=CLF_i,
                   dt_out='tox',dt_in=dt,n_ds=i_ds,
...
def evalPred(Y_truth,Y_inferred,post=None):
    M = dict(sens=recall_score(Y_truth,Y_inferred),
             ...
             ppv=ppv_calc(Y_truth,Y_inferred),
             npv=npv_calc(Y_truth,Y_inferred),
...
def summarizePerf(P):
    ...
    Agg = dict(bacc=dict(mn=np.mean,sd=np.std),
               ...
               ppv=dict(mn=np.mean,sd=np.std),
               npv=dict(mn=np.mean,sd=np.std),
...
def predPerfSummary2(tox,Col_ml=None,Col_sum=None,limit=1000):
    P_cvt=[]
    P_trn=[]
    ...
    # CVT performance
    X1 = pd.DataFrame(P_cvt)
    agg1=dict(f1_mn=np.mean,bacc_mn=np.mean,acc_mn=np.mean,sens_mn=np.mean,spec_mn=np.mean,ppv_mn=np.mean,npv_mn=np.mean)
    by0 = ['pred','lr','dt_in','n_obs','n_pos','n_neg','n_ds']
    Perf_cvt=X1.groupby(by=by0).aggregate(agg1)
    for x in ['acc','bacc', 'f1', 'sens', 'spec','ppv','npv']:
        Perf_cvt['%s_sd' % x]=X1.groupby(by=by0).apply(lambda g: pooledStdev(g,x))
...
def pooledStdev(g,x):
    ...
    return np.sqrt(((N-1)*A_sd**2).sum()/(N-1).sum())
